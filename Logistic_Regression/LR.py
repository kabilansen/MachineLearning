...
for i in range(rows):
    if(trainlabel_matrix.get(i) !=None):
        ...
        prev_error += (-trainlabel_matrix.get(i)*math.log(sigmoid)) - ((1-trainlabel_matrix.get(i))*math.log(1-sigmoid))
flag = 0
k=0
while(flag != 1):
    k+=1
    ...
    for i in range(rows):
        if(trainlabel_matrix.get(i) != None):
            ...
            sigmoid = 1/(1+math.exp(-1*dp))
            for j in range(cols):
                    delf[j] += (-trainlabel_matrix.get(i) + sigmoid)*data_file_matrix[i][j]
    # prev_error = math.log(1 + math.exp(-ydp))
    for j in range(cols):
        w[j] -= eta*delf[j]
    error = 0
    for i in range(rows):
        if(trainlabel_matrix.get(i) != None):
            ...
            #error += math.log(1 + math.exp(-ydp))
            error += (-trainlabel_matrix.get(i)*math.log(sigmoid)) - ((1-trainlabel_matrix.get(i))*math.log(1-sigmoid))
            eta = changeEta(eta, abs(prev_error-error))

    #print(dp)                    
    #print(k, error, eta)
    if(abs(prev_error-error) <= 0.00000001):
        flag = 1
        #print(w)
    prev_error = error
